...
def main(cfg: DictConfig):
    ...
    if accelerator.is_local_main_process and True:
        ...
        # helper: uniformly select up to k indices from range(total)
        def select_uniform_indices(total: int, k: int):
            if total <= 0:
                return []
            k = min(k, total)
            if k == 1:
                return [0]
            # ensure first is 0 and last is total-1
            return [int(round(i * (total - 1) / (k - 1))) for i in range(k)]

        n_steps = cfg.gm.n_discretization_steps
        total_frames = n_steps + 1
        sel = select_uniform_indices(total_frames, 8)
