...
def get_model(cfg: DictConfig, model=None, final_model_ckpt_path=None):
    ...
    from mlkit.util.mask.masker import Masker as OneDimMasker
    ...
    model_config = EuclideanVPSDEConfig(
        ...
        model_uses_continuous_time=cfg.diffusion.get("model_uses_continuous_time", False),
    )
    ...
    classifier_conditioner = build_classifier_conditioner(
        ...
        guidance_scale=cfg.diffusion.gs,
    )
    sampling_hook = diffuser.get_condition_pre_update_in_step_fn_hook([classifier_conditioner])
    sampling_hook_handlers = diffuser.register_hooks([sampling_hook])
