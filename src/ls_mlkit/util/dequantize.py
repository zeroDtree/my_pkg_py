...
def get_float_weight(model: torch.nn.Module):
    ...
    Returns:
        torch.Tensor: the float weight of the model
    """
...
def replace_module_with_linear(model: torch.nn.Module, target) -> None:
    ...
    for name, module in model.named_children():
        if isinstance(module, target):
            ...
            with torch.no_grad():
                ...
                if bias:
                    new_module.bias.data = module.bias if module.bias is not None else None
...
def dequantize(model, dtype) -> None:
    """Dequantize a model
