...
from ..util.base_class.base_gm_class import GMHookStageType
...
class EuclideanDiffuser(BaseDiffuser):
    ...
    def get_posterior_mean_fn(
        ...
        """Return a callable that computes E[x_0 | x_t] for this diffuser.

        Args:
            score: pre-computed unconditional score tensor (optional)
            score_fn: callable that produces the score on demand (optional)
            batch: full batch dict forwarded from the caller (optional)

        Returns:
            Callable: posterior mean function with signature
                ``(x_t, t, padding_mask) -> Tensor``
        """
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            for u in range(1, n_repaint_steps + 1):
                ...
                if u < n_repaint_steps and (t > 0).all() and i + 1 < len(timesteps):
                    prev_t = torch.ones(tuple(macro_shape), device=device, dtype=torch.long) * timesteps[i + 1]
    ...
    def recover_bright_region(
        ...
        **kwargs,
        ...
        x_0 = x_known
        # Rebuild from macro shape (t already has micro dims); avoid ones_like(t)
        # which would double-expand after complete_micro_shape.
        macro_shape = t.shape[: -self.config.ndim_micro_shape]
        t_a = torch.ones(tuple(macro_shape), device=t.device, dtype=torch.long) * (
            self.time_scheduler.get_timestep_index_start() - 1
        )
        t_a = self.hook_manager.run_hooks(
            stage=GMHookStageType.POST_SAMPLING_TIME_STEP,
    ...
    def _setup_conditioners(
        ...
        *,
        ...
        Shared helper used by all Euclidean diffuser subclasses.  Builds the
        posterior-mean function from the current unconditional score prediction,
        then calls ``prepare_condition_dict`` / ``set_condition`` on every
        enabled conditioner.
