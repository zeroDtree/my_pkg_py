...
class EuclideanDDIMConfig(EuclideanDDPMConfig):
    """
    Config Class for Euclidean DDIM Diffuser
    """
...
class EuclideanDDIMDiffuser(EuclideanDDPMDiffuser):
    ...
    def step(
        ...
        **kwargs: Any,
        ...
        # Predict noise or x_0 depending on mode
        if mode == "epsilon":
        ...
        # x_hat_0 = (x_t - sqrt(1 - alpha_bar_t) * epsilon) / sqrt(alpha_bar_t)
        ...
        # sigma = eta * sqrt((1 - alpha_bar_{t-1}) / (1 - alpha_bar_t) * (1 - alpha_bar_t / alpha_bar_{t-1}))
        sigma2 = self.get_sigma2(t, prev_t)
        sigma = config.eta * torch.sqrt(sigma2)

        # direction = sqrt(1 - alpha_bar_{t-1} - sigma^2) * epsilon
        direction = torch.sqrt(1 - alpha_prod_t_prev - sigma**2) * epsilon_predicted

        # x_{t-1} = sqrt(alpha_bar_{t-1}) * x_hat_0 + direction + sigma * z
